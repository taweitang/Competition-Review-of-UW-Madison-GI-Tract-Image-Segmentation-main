...
def main(cfg):
    ...
    with open(cfg.data_json_dir, "r") as f:
        ...
        for i in range(len(cfg.data_json["val"])):
            
            temp = cfg.data_json["val"][i]['image']
            temp = temp.replace('/kaggle/working','./workspace/data/uw-gi-tract')
            cfg.data_json["val"][i]['image'] = temp
            temp = cfg.data_json["val"][i]['mask']
            temp = temp.replace('/kaggle/working','./workspace/data/uw-gi-tract')
            cfg.data_json["val"][i]['mask'] = temp
            temp = cfg.data_json["val"][i]['mask_multiclass']
            temp = temp.replace('/kaggle/working','./workspace/data/uw-gi-tract')
            cfg.data_json["val"][i]['mask_multiclass'] = temp

        for i in range(len(cfg.data_json["train"])):
            
            temp = cfg.data_json["train"][i]['image']
            temp = temp.replace('/kaggle/working','./workspace/data/uw-gi-tract')
            cfg.data_json["train"][i]['image'] = temp
            temp = cfg.data_json["train"][i]['mask']
            temp = temp.replace('/kaggle/working','./workspace/data/uw-gi-tract')
            cfg.data_json["train"][i]['mask'] = temp
            temp = cfg.data_json["train"][i]['mask_multiclass']
            temp = temp.replace('/kaggle/working','./workspace/data/uw-gi-tract')
            cfg.data_json["train"][i]['mask_multiclass'] = temp
...
def run_eval(model, val_dataloader, post_pred, metric_function, seg_loss_func, cfg, epoch):
    ...
    all_score = dice_score * 0.5 + hausdorff_score * 0.5
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--fold", type=int, default=3, help="fold")
    parser.add_argument("-s", "--seed", type=int, default=20220421, help="seed")
    parser.add_argument("-w", "--weights", default='/content/drive/MyDrive/3d_training_pipeline/output/unet_3d_multilabel_large_net/fold3/best_weights.pth', help="the path of weights")
